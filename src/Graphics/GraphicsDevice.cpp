...
{
//-----------------------------------------------------------------------------
// 要求する機能レベル (Feature Level)
//
//   ■ 機能レベルとは
//     GPU が「どの世代の機能まで対応しているか」を段階で表したもの。
//     D3D_FEATURE_LEVEL_11_0 は DirectX 11 世代（2009 年頃）の機能に相当します。
//
...
//-----------------------------------------------------------------------------
// デストラクタ
//   ComPtr がメンバなので、解放処理は自動で行われます（明示的な Release は不要）。
...
//-----------------------------------------------------------------------------
// EnableDebugLayer : デバッグレイヤーを有効にする
//
...
//
//   ■ 有効化に失敗するケース
//     Windows の「オプション機能」に “Graphics Tools” が入っていないと
//     D3D12GetDebugInterface が失敗します。その場合でもアプリは動作するため、
//     ここでは例外を投げず警告ログに留めます。
...
//-----------------------------------------------------------------------------
// ConfigureInfoQueue : デバッグメッセージの扱いを設定する
//
//   ID3D12InfoQueue は、デバッグレイヤーが検出した問題を溜めておくキューです。
//   SetBreakOnSeverity を設定しておくと、深刻度の高いメッセージが出た瞬間に
//   デバッガのブレークポイントが発動します。
//
//   これが極めて重要な理由:
//     設定しないと、エラーは出力ウィンドウに流れるだけで実行は続行されます。
//     その結果「原因の場所」ではなく「はるか後の別の場所」でクラッシュし、
//     原因究明が非常に困難になります。
