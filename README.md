...
現在の到達点：**市松模様のテクスチャを貼った立方体が、透視投影の付いた 3D 空間で回転する**

![座標変換のパイプライン](docs/assets/transform-pipeline.svg)

カメラ（ビュー行列）と透視投影を入れ、6 面それぞれに UV を持つ立方体を
インデックスバッファで描いています。
背面カリングと深度テストの両方が効いているため、立体として正しく見えます。
...
│       ├── MeshPipeline.*            ルートシグネチャ・PSO・頂点/インデックス/定数バッファ
