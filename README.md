...
現在の到達点：**モデルの形・色・テクスチャをファイルから読み込み、影と映り込み付きで自由に見回せる**

![IBL を入れたシーン](docs/assets/ibl-scene.png)
...
色は **リニア空間** で計算し、入出力だけ sRGB に変換しています。
明るさが 1.0 を超える部分はトーンマッピングで押し込むため、白飛びしません。

陰影は **物理ベース (PBR)** で、材質は金属度と粗さの 2 つで表しています。
さらに **IBL（環境マップ）** を入れてあり、周囲の景色が金属に映り込みます。
影の中が青いのは、空の色で照らされているからです。
