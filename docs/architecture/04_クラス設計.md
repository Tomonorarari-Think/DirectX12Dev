...
flowchart TD
    ...
    subgraph GFX["Graphics / 　DirectX に依存する層"]
        ...
        Pipe["TrianglePipeline<br/>PSO・頂点/定数バッファ"]
    ...
    style Renderer fill:#ddf4ff,stroke:#0969da
    style Window fill:#dafbe1,stroke:#1a7f37
```

**依存は一方向**です。矢印が循環していないことがポイントです。

- `Window` は DirectX を一切知りません（描画側を差し替えても無変更）
- `GraphicsDevice` / `CommandQueue` / `SwapChain` / `DepthBuffer` / `TrianglePipeline`
  は互いを知りません
...
（[03 章 ポイント 6](03_初期化と1フレームの流れ.md#ポイント-6-深度バッファは-1-枚でよい)）。
...
| 追加したい機能 | 置き場所 |
|--------------|---------|
| 複数モデルの管理 | `TrianglePipeline` を `MeshPipeline` に発展させる |
