...
0 が非金属、1 が金属です。**中間の値に物理的な意味はありません**。
塗装が剥げた金属のように「場所によって違う」ものをテクスチャで表すために存在します。

金属度によって基本色の役割が入れ替わります。
非金属では拡散反射の色、金属では**反射そのものの色**になります。

### 粗さ (Roughness)

0 が鏡のよう、1 がざらざらです。
微小な鏡（マイクロファセット）の向きがどれだけ散らばっているかを表します。

### マイクロファセット BRDF

表面を「向きの揃っていない無数の微小な鏡」の集まりとみなすモデルです。
反射の量は **D（分布）× G（幾何減衰）× F（フレネル）** の積で表されます。

### GGX（トロウブリッジ・ライツ）

微小な鏡の向きの分布を表す関数です。
ハイライトの中心が鋭く、裾が長く伸びます。実測に近いとされ、現在の標準です。

### フレネル (Fresnel)

浅い角度から見るほど反射が強くなる現象です。
水面を真上から見ると底が見え、遠くの水面は空を映すのがこれです。

### エネルギー保存

入ってきた以上の光は出せない、という性質です。
反射した割合 (F) の残りだけが内部へ入って拡散反射になるため、
「光っている所ほど下地の色が薄い」という自然な見え方になります。

### IBL (Image Based Lighting)

周囲の景色を写した画像を光源として使う手法です。
**金属は映り込みだけで見えている**ため、これが無いと真っ黒になります。

### 環境マップ

周囲 360° の景色を収めた画像です。本プロジェクトでは
**正距円筒図法**（世界地図と同じ畳み方）の 2D テクスチャ 1 枚を使います。
キューブマップ（6 面の立方体）のほうが歪みは少なく、実務では主流です。

### 正距円筒図法 (Equirectangular)

横に方位角、縦に天頂からの角度を割り当てる畳み方です。
`v = 0` が真上、`v = 1` が真下。左右の端はつながっています。

**極付近は横に引き伸ばされます。** 足し合わせるときは
立体角の補正（`sin θ`）が要ります。

### イラディアンス マップ

「面がこの向きを向いているとき、半球全体からどれだけ光が入るか」を
方向ごとに焼いた画像です。拡散反射はこれにアルベドを掛けるだけで求まります。
結果はなだらかなので、32×16 程度の小さな画像で足ります。

### 事前フィルタリング (Prefiltering)

毎フレームやると重すぎる積分を、起動時に 1 回だけ済ませて画像に焼くことです。
IBL では拡散用（イラディアンス マップ）と鏡面用（粗さ別のミップ列）の
2 種類を作ります。

### ミップ列 / サブリソース

1 つのテクスチャリソースの中に並ぶ、段階的に小さい画像の列です。
段ごとに転送先が分かれており、これを**サブリソース**と呼びます。
配置は `GetCopyableFootprints` に計算してもらいます。

IBL では「粗さ → 段」の対応で使うため、自動で段を選ぶ `Sample` ではなく
段を明示する **`SampleLevel`** で読みます。

### ブレンディング（合成）

`結果 = 前景 × SrcBlend (op) 背景 × DestBlend` の式で、
すでに書かれている色と混ぜることです。

| 合成 | 式 | 用途 |
|------|-----|------|
| アルファ | `前景 α + 背景 (1-α)` | 煙・ガラス |
| 加算 | `前景 + 背景` | 光・炎。**暗くならず、順番も選ばない** |
| 乗算 | `前景 × 背景` | 影・汚れ |

### 乗算済みアルファ

色にアルファをあらかじめ掛けておく持ち方です。`SrcBlend` が `ONE` で
済むので、**アルファ合成と加算合成を `DestBlend` だけで切り替え**られます。
透明な部分の色がにじまない利点もあります。

### ビルボード

常にカメラを向く板です。モデル行列で回すのではなく、
**カメラの右方向と上方向で板を張る**のがふつうです。

### ディゾルブ

ノイズのしきい値でピクセルを捨て、溶けて消えるように見せる表現です。
**ポリゴンは減りません。** `clip` で「描かない」と決めているだけです。

### `clip`

「引数が負なら、このピクセルの処理をここで打ち切る」HLSL の命令です。
色も深度も書き込まれません。重い処理の前に置くほど得ですが、
**早期深度テストが無効になる**ため、使う場面は絞ります。

### ポストプロセス（後処理）

描き終えた絵をもう一度読んで加工することです。物体を描くシェーダーは
「そのピクセル 1 点」しか知らないため、隣を見る処理や画面全体を見る処理は
描き終えてからでないと行えません。

### 中間バッファ / オフスクリーン描画

画面ではなくテクスチャへ描くことです。**HDR（浮動小数点）**で持たないと、
書いた時点で 1.0 を超えた明るさが切り落とされます。

### ブルーム

強い光のまわりがにじんで見える効果です。
しきい値抽出 → 横ぼかし → 縦ぼかし → 合成、の順で作ります。
**圧縮（トーンマッピング）の前に足す**のが大事です。

### 分離可能フィルタ

2 次元のぼかしを、横 1 回 ＋ 縦 1 回に分解できる性質です。
ガウス関数はこれを満たすため、9×9 のぼかしが 81 回ではなく 18 回で済みます。

### ビネット（周辺減光）

画面の隅を暗くして、中央へ目を向けさせる効果です。
**縦横比を掛けないと**、横長の画面で減光の形が楕円になります。

### ミップマップ

同じ絵を段階的に縮小した画像の列です。GPU が「1 ピクセルがテクスチャ上で
どれだけの広さを覆うか」から段を選び、**先に平均してある値**を読みます。
容量は元の 1/3 増えるだけです（1 + 1/4 + 1/16 + … = 4/3）。

### エイリアシング

細かい変化を粗い格子で表そうとして失敗し、元には無い模様やちらつきが
現れる現象です。テクスチャの縮小で起きるものはミップマップで防げますが、
多角形の輪郭のギザギザは別の問題（MSAA などの領分）です。

### 異方性フィルタ (Anisotropic Filtering)

縦と横で縮小率が違う場所（斜めに見た床など）で、細長い範囲を何点も読む
フィルタです。ミップだけだと「きつく縮む向き」に合わせて段が選ばれ、
**必要以上にぼけます**。`MaxAnisotropy` に 1 より大きい値を指定します。

### スカイボックス

背景に周囲の景色を描く手法です。本プロジェクトでは立方体も球も使わず、
**画面いっぱいの三角形 1 枚**で描きます。

### `SV_VertexID`

頂点シェーダーが「自分が何番目の頂点か」を受け取るための
システム値セマンティクスです。これがあれば、頂点バッファを用意せずに
座標を計算で作れます。入力レイアウトも空でかまいません。

### 全画面三角形 (Full-screen Triangle)

画面より大きい三角形 1 枚で画面全体を覆う手法です。
四角形（三角形 2 枚）だと対角線上のピクセルが 2 度処理されますが、
三角形 1 枚なら重なりが出ません。はみ出した部分は切り落とされます。

### 法線マップ

ピクセルごとの法線をテクスチャから読む手法です。陰影は法線だけで決まるため、
**頂点を増やさずに凹凸があるように見せられます**。面の形そのものは
変わらないので、**輪郭は平らなまま**です。

### 接線空間 (Tangent Space)

面ごとに決まる 3 本の軸で作る座標系です。法線マップはこの空間での
傾きを記録します。ワールド空間で記録しないのは、モデルが回転したときに
作り直しになるためです。
...
傾いていない場所は `(0, 0, 1)`。8 bit に詰めると **(128, 128, 255)** に
なるため、法線マップは一様に青紫に見えます。

### 従接線の符号 (Handedness)

B は `cross(N, T)` で計算できますが、**UV が鏡像になっている面**では
向きが裏返ります。そのため `+1` / `-1` の符号を接線と一緒に持ちます。
間違えると凹凸が凸凹に反転します。

### 分割和近似 (Split-sum Approximation)

環境光の鏡面反射の積分を、「ぼかした景色」と「粗さ・角度で決まる係数」の
2 つの積に分けて近似する手法です。後者は本来 2D の事前計算テクスチャ
（BRDF LUT）ですが、多項式で近似することもできます。
...
| IBL | 周囲の景色 1 枚を光源として扱う手法。金属が金属らしく見える |
| 正距円筒図法 | 全方位を 1 枚に畳む方法。横が方位角、縦が天頂角 |
| イラディアンス マップ | 向きごとの「入ってくる光の合計」を焼いた画像 |
| ミップ列 | 段階的に小さい画像の列。粗さに応じたぼけを表すのに使う |
| 法線マップ | 法線をテクスチャから読む。頂点を増やさず凹凸を出す |
| 接線空間 | 面の上の 3 本の軸（N・T・B）。法線マップの基準 |
| スカイボックス | 背景に周囲の景色を描くこと |
| SV_VertexID | 頂点の通し番号。頂点バッファ無しで座標を作れる |
| ミップマップ | 段階的に縮小した画像の列。縮小時のちらつきを防ぐ |
| エイリアシング | 細かい変化を粗い格子で表そうとして生じる偽の模様 |
| 異方性フィルタ | 斜めに見た面が必要以上にぼけるのを防ぐフィルタ |
| ポストプロセス | 描き終えた絵をもう一度読んで加工すること |
| 中間バッファ | 画面の代わりに描き込むテクスチャ。HDR で持つ |
| ブルーム | 強い光のまわりがにじむ効果 |
| ディゾルブ | ノイズのしきい値でピクセルを捨て、溶けて消えるように見せる |
| clip | 引数が負ならピクセルの処理を打ち切る HLSL の命令 |
| ブレンディング | すでに書かれている色と混ぜること |
| 加算合成 | 足すだけの合成。暗くならず、描く順番を選ばない |
| ビルボード | 常にカメラを向く板 |
...
| 金属度 | 0 が非金属、1 が金属。基本色の役割が入れ替わる |
| 粗さ | 0 が鏡、1 がざらざら |
| フレネル | 浅い角度ほど反射が強くなる現象 |
| IBL | 周囲の景色を光源として使う手法。金属に必須 |
| PSO | 描画設定の全部入りパック |
