...
flowchart TD
    ...
    style D fill:#fff8c5,stroke:#8a6d00,color:#1f2328
...
[INFO ] シェーダーを読み込みます: E:\Claude\DirectX12Dev\shaders\Mesh.hlsl
...
| ウィンドウは出るが真っ黒 | 描画されていない | [05_つまずきポイント集](../troubleshooting/症状別の原因逆引き.md) を参照 |
...
> **構文エラーで済むとは限りません。**
> 文字化けの過程で改行が失われると、コメントの次の行がコメントに飲み込まれます。
> するとその行は**エラーも出さずに実行されません**。
> 「なぜかこの処理だけ動かない」という形で現れるので、原因にたどり着きにくい種類の事故です。
> 新しく `.ps1` を作るときも、最初から BOM 付きで保存してください。
