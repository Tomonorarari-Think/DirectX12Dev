...
本プロジェクトでは共通処理を
[`UploadHelper`](../../src/Graphics/UploadHelper.h) に切り出し、
テクスチャと頂点バッファの両方から使っています。

```cpp
// 頂点バッファ（バッファなので行ピッチの整列は不要。一度に memcpy できる）
m_vertexBuffer = upload::CreateBufferWithData(
    device, commandQueue, kTriangleVertices, size,
    D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);

// テクスチャ（行ピッチの整列があるので、記録処理だけを渡す）
upload::ExecuteImmediate(device, commandQueue, [&](ID3D12GraphicsCommandList* cl) {
    cl->CopyTextureRegion(&destination, 0, 0, 0, &source, nullptr);
    cl->ResourceBarrier(1, &barrier);
});
```

**バッファとテクスチャの違いは 1 点だけです。**
バッファには行ピッチの整列が無いので、画像のように 1 行ずつではなく
一度に `memcpy` できます。
