...
準備が整ったので、実際に絵を出します。
対応するコードは [`src/Graphics/MeshPipeline.h`](../../src/Graphics/MeshPipeline.h) /
[`.cpp`](../../src/Graphics/MeshPipeline.cpp)、
シェーダーは [`shaders/Mesh.hlsl`](../../shaders/Mesh.hlsl) です。
...
```mermaid
flowchart LR
    A["C++ の struct Vertex<br/>MeshPipeline.h"] --- B["入力レイアウト<br/>D3D12_INPUT_ELEMENT_DESC"]
    B --- C["HLSL の VSInput<br/>Mesh.hlsl"]
