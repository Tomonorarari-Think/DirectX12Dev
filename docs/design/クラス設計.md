...
│       ├── MeshPipeline.h / .cpp    ルートシグネチャ・PSO・頂点/インデックス/定数バッファ
...
flowchart TD
    ...
    subgraph GFX["Graphics / 　DirectX に依存する層"]
        ...
        Pipe["MeshPipeline<br/>PSO・頂点/定数バッファ・ライト"]
...
| 項目 | 内容 |
|------|------|
| 責務 | ルートシグネチャ、PSO、頂点／インデックス／定数バッファの生成と、変換行列・ライト情報の計算、描画命令の記録 |
| 提供するもの | `Initialize()` / `Update(frameIndex, viewProjection, cameraPosition, time)` / `RecordDrawCommands(commandList, frameIndex)` |
...
| 影を落とす | `Graphics/ShadowMap.h` （深度バッファを SRV としても読む） |
