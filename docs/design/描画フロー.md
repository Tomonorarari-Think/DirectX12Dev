...
このプロジェクトが起動してから立方体が表示されるまで、何がどの順番で起きているかを追います。
...
flowchart TD
    ...
    style F fill:#dafbe1,stroke:#0d4f21,color:#1f2328
...
flowchart TD
    ...
    SH["⑧'' シェーダー可視ディスクリプタヒープの生成<br/>（SRV を置く場所。同時 1 本しか設定できないので共有する）"]
    S9["⑨ コマンドアロケータ ＋ コマンドリストの生成"]
    S10["⑩ ルートシグネチャの生成"]
    S11["⑪ シェーダーのコンパイル → PSO の生成"]
    S12["⑫ 定数バッファの生成<br/>（フレーム共通・オブジェクト別）"]
    S13["⑬ メッシュの生成<br/>（立方体と床、DEFAULT ヒープへ転送）"]
    S13c["⑬'' シャドウマップの生成<br/>TYPELESS で作り、DSV と SRV を張る"]
    S13b["⑬' テクスチャの生成<br/>UPLOAD → DEFAULT へ転送し、SRV をヒープに登録"]
    S14["⑭ ビューポート / シザー矩形の設定"]

    S1 --> S2 --> S3 --> S4 --> S5 --> S6 --> S7 --> S8 --> S8b --> SH --> S13c --> S9 --> S10 --> S11 --> S12 --> S13 --> S13b --> S14
    ...
    S8b -. 深度の形式が必要 .-> S11
    SH -. SRV の置き場が必要 .-> S13b
    S6 -. 転送コマンドの実行に必要 .-> S13b

    style S1 fill:#fff8c5,stroke:#8a6d00,color:#1f2328
    style S4 fill:#ddf4ff,stroke:#0a4f9e,color:#1f2328
    style S7 fill:#fff8c5,stroke:#8a6d00,color:#1f2328
    style S11 fill:#fff8c5,stroke:#8a6d00,color:#1f2328
```
...
| ⑧'' | [`DescriptorHeap::Initialize`](../../src/Graphics/DescriptorHeap.cpp) |
| ⑨ | [`Renderer::CreateCommandObjects`](../../src/Graphics/Renderer.cpp) |
| ⑩〜⑫、⑬' | [`MeshPipeline::Initialize`](../../src/Graphics/MeshPipeline.cpp) |
| ⑬ | [`Renderer::CreateSceneMeshes`](../../src/Graphics/Renderer.cpp) |
| ⑬' の中身 | [`Texture2D::Initialize`](../../src/Graphics/Texture2D.cpp) |
...
flowchart TD
    ...
    subgraph CPU["CPU 側の作業（命令の記録だけ。GPU はまだ動かない）"]
        ...
        C2b["(2b) 定数バッファに書き込む<br/>フレーム共通（カメラ・ライト・光源の行列）<br/>＋ オブジェクト別（行列）"]
        C2d["(2d) 第 1 パス : シャドウマップへ深度を描く<br/>バリア → クリア → 影用 PSO → 全メッシュ → バリア"]
        C3["(3) バリア　PRESENT → RENDER_TARGET"]
        C4["(4) RSSetViewports / RSSetScissorRects"]
        C4b["(4b) SetDescriptorHeaps<br/>シェーダー可視ヒープを設定（テーブル設定より前に必須）"]
        C5["(5) OMSetRenderTargets　描画先（RTV ＋ DSV）"]
        C6["(6) ClearRenderTargetView　背景を塗る"]
        C6b["(6b) ClearDepthStencilView　深度を 1.0 で埋める"]
        C7["(7) MeshPipeline::Bind　PSO・ルートシグネチャ<br/>フレーム定数（カメラ・ライト）・テクスチャ<br/>→ オブジェクトごとに BindObject して<br/>DrawIndexedInstanced　床と立方体を描く"]
        C8["(8) バリア　RENDER_TARGET → PRESENT"]
        C9["(9) commandList->Close()　記録終了"]
        C1 --> C2 --> C2b --> C2d --> C3 --> C4 --> C5 --> C6 --> C6b --> C7 --> C8 --> C9
    end
    ...
    style W0 fill:#fff8c5,stroke:#8a6d00,color:#1f2328
    style G10 fill:#dafbe1,stroke:#0d4f21,color:#1f2328
    style G12 fill:#ddf4ff,stroke:#0a4f9e,color:#1f2328
```
...
### ポイント 4: SetDescriptorHeaps は SetGraphicsRootDescriptorTable より前に
...
これもコマンドリストを `Reset` するたびに消えるため、毎フレーム必要です。

### ポイント 5: 深度バッファのクリアを忘れない
...
flowchart LR
    ...
    style P0 fill:#fff1e5,stroke:#7a3200,color:#1f2328
    style P1 fill:#fff1e5,stroke:#7a3200,color:#1f2328
    style P2 fill:#fff1e5,stroke:#7a3200,color:#1f2328
```
...
- 描いているのは **三角形 1 枚とクリアだけ**（計測当時）。GPU の仕事は数マイクロ秒で終わる
