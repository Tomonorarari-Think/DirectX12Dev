...
flowchart TD
    ...
    S12["⑫ 頂点／インデックスバッファの生成<br/>（UPLOAD → DEFAULT へ転送）"]
    S13["⑬ 定数バッファの生成<br/>（バックバッファ枚数ぶんの領域）"]
    S13b["⑬' テクスチャの生成<br/>UPLOAD → DEFAULT へ転送し、SRV をヒープに登録"]
    S14["⑭ ビューポート / シザー矩形の設定"]

    S1 --> S2 --> S3 --> S4 --> S5 --> S6 --> S7 --> S8 --> S8b --> SH --> S9 --> S10 --> S11 --> S12 --> S13 --> S13b --> S14
...
flowchart TD
    ...
    subgraph CPU["CPU 側の作業（命令の記録だけ。GPU はまだ動かない）"]
        ...
        C2b["(2b) 定数バッファ[frameIndex] に変換行列とライト情報を書き込む"]
        ...
        C7["(7) SetPipelineState / IASetVertexBuffers<br/>IASetIndexBuffer<br/>SetGraphicsRootConstantBufferView（行列とライト）<br/>SetGraphicsRootDescriptorTable（テクスチャ）<br/>DrawIndexedInstanced(36, 1, 0, 0, 0)　立方体を描く"]
        C8["(8) バリア　RENDER_TARGET → PRESENT"]
        C9["(9) commandList->Close()　記録終了"]
        C1 --> C2 --> C2b --> C3 --> C4 --> C4b --> C5 --> C6 --> C6b --> C7 --> C8 --> C9
    end
