...
flowchart TD
    ...
    subgraph CPU["CPU 側の作業（命令の記録だけ。GPU はまだ動かない）"]
        ...
        C7["(7) SetPipelineState / IASetVertexBuffers<br/>SetGraphicsRootConstantBufferView（行列）<br/>SetGraphicsRootDescriptorTable（テクスチャ）<br/>DrawInstanced(9, 1, 0, 0)　三角形 3 枚を描く"]
