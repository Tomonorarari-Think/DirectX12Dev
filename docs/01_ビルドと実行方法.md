...
```
Ctrl+Shift+B
  → .vscode/tasks.json の "build (Debug x64)"
    → tools/build.ps1
      → vswhere.exe で MSBuild.exe の場所を検索
        → MSBuild.exe DirectX12Dev.vcxproj /p:Configuration=Debug /p:Platform=x64
```
...
| ウィンドウは出るが真っ黒 | 描画されていない | [05_つまずきポイント集](05_つまずきポイント集.md) を参照 |
